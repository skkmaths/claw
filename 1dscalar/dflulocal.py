...
def local_speed_f(ul, ur):
    ...
    return abs(dxf(res.x))


def local_speed_g(ul, ur):
    a = min(ul, ur)
    b = max(ul, ur)
...
def llfd(x, ul, ur, fl, fr, lamda, h, sl, sr):
    ...
    if x < 0.0:
        speed = local_speed_g(ul, ur)
        return 0.5*( flux(x, ul) + flux(x, ur) - speed * ( ur  - ul )  )
    elif x >0.0:
        speed = local_speed_f(ul, ur)
        return 0.5*( flux(x, ul) + flux(x, ur) - speed * ( ur  - ul )  )
    else:
        speed = max(local_speed_g(kval, ur), local_speed_f(ul, kval))
