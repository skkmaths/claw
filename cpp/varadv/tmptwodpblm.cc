...
{ 
    ...
    // left vertical
    for (unsigned int j = 2; j < grid.ny + 2; ++j){
        //Fn = xflux(  grid.x(0,j-2), grid.y(0,j-2)+0.5*grid.dy, exact ( grid.x(0,j-2), grid.y(0,j-2)+0.5*grid.dy, time ))
          int i = 1;
          double x = grid.x(i-1,j-2);
          double y = grid.y(i-1,j-2)+grid.dy/2.0;
          double sl = reconstruct(sol(i+1,j),sol(i+1,j), sol(i+1,j));
          double sr = reconstruct(sol(i+2,j), sol(i+1,j), sol(i+1,j));
          double Fn = xnumflux(x, y, sl, sr);
          res(i,j) +=  lam_x * Fn;
          res(i+1,j)-=  lam_x * Fn;
    }
    ...
    // Bottom face
    for (unsigned int i = 2; i < grid.nx + 2; ++i){
        double Gn = yflux(grid.x(i-2, 0)+ 0.5*grid.dx, grid.y(i-2,0), exact( grid.x(i-2, 0)+ 0.5*grid.dx, grid.y(i-2,0), time ) );
        res(i,2) -= lam_y * Gn;
    }
